...
class MishkahRealtimeStore extends EventEmitter {
  ...
  #applySnapshot(message) {
    ...
    for (const [moduleId, moduleSnapshot] of Object.entries(snapshot)) {
      ...
      if (this.dbAdapter) {
        this.#cacheModuleState(moduleId, moduleSnapshot, message.meta);
      }
    }
  ...
  #applyEvent(event) {
    ...
    if (this.dbAdapter) {
        this.#cacheModuleState(moduleId, moduleState, { lastEventId: event.eventId, version: moduleState.version });
    }
