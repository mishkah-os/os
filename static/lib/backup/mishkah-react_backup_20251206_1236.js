...
(function (root, factory) {
    ...
    function performPatch(f) {
        ...
        var parent = f.dom.parentNode;
        var oldDom = f.dom;
        var newDom;
        try {
            newDom = renderVNode(nextVNode);
            parent.replaceChild(newDom, oldDom);
            f.dom = newDom;
            f.vnode = nextVNode;
        } catch (e) {
            console.error('[MR:Patch] Render error', e);
        }
        f.dirty = false;
        activeFiberId = null;
        runLayoutEffects();
        ...
        var setter = (function (cId, hIdx) {
            return function setValue(newValue) {
                ...
                if (next !== current) {
                    ...
                    var fiber = fiberRegistry.get(activeFiberId || cId);
                    if (fiber && fiber.mounted) {
