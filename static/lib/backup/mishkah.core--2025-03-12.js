...
(function (root, factory) {
  ...
  var VDOM = (function () {
    ...
    function normalizeChildren(children) {
      ...
      for (var i = 0; i < arr.length; i++) {
        ...
        // Null Handling: Convert null/false to comment node
        if (c == null || c === false) {
          out.push({
            _type: 'comment',
    ...
    function h(tag, category, config, children) {
      ...
      // Top-level Props (gkey/key outside attrs)
  ...
  // 1. Flatten DSL (D.Div instead of D.Containers.Div)
  var flatDSL = {};
  for (var cat in hAtoms) {
    flatDSL[cat] = hAtoms[cat];
    for (var tag in hAtoms[cat]) {
      if (!flatDSL[tag]) {
        flatDSL[tag] = hAtoms[cat][tag];
      }
    }
  }

  M.DSL = assign({ h: VDOM.h }, flatDSL);
  M.h = VDOM.h; // 2. Expose h() function directly
  M.Head = Head;
  M.app = App;
